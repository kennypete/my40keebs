...
bool rgb_matrix_indicators_user(void) {
    ...
    // Special treatment of layer three, which has only the Reset key
    // (keeping fixed-brightness deliberately as a safety indicator, not part
    // of the colour scheme).  The Reset key and the Lower/Raise keys are
    // all made RGB_RED so that it is really clear we are in a danger zone.
    if (layer == 3) {
        rgb_matrix_set_color_all(RGB_OFF);

        uint8_t led_index = g_led_config.matrix_co[0][4];   // RESET (QK_BOOT)
        if (led_index != NO_LED) {
